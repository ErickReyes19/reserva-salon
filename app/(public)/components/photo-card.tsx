...
import { getGaleriasByFotografoId } from "@/app/(protected)/fotografos/[id]/galeria/actions"
import PhotographerGalleryDialog from "./dialogFografo"
...
export default function PhotoCard({ fotografo }: { fotografo: Fotografo }) {
  ...
  const handleClick = async () => {
    const galeria = await getGaleriasByFotografoId(fotografo.id!)
    setFotos(galeria)
    setOpen(true)
  }
  ...
  return (
    <>
      <Card
        onClick={handleClick}
        className="overflow-hidden border border-gray-100 shadow-lg hover:shadow-xl transition-all duration-300 h-full group rounded-xl cursor-pointer"
      >
